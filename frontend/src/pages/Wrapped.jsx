  ...
  const slides = data && !data.empty ? [
    // Slide 0: Hero
    {
      ...
      content: (
        <div className="text-center">
          ...
          <h2 className="text-4xl font-black mb-2">Tu {year}</h2>
          <h3 className="text-xl text-cine-muted">en películas</h3>
          <div className="mt-8 grid grid-cols-2 gap-4">
            <div className="bg-white/5 rounded-2xl p-4">
              <p className="text-3xl font-black text-cine-accent">{data.total_movies}</p>
              <p className="text-xs text-cine-muted">Películas vistas</p>
            </div>
            <div className="bg-white/5 rounded-2xl p-4">
              <p className="text-3xl font-black text-cine-gold">{data.estimated_hours}h</p>
              <p className="text-xs text-cine-muted">Horas estimadas</p>
            </div>
          </div>
        </div>
      ),
    ...
    // Slide 1: Top genre
    {
      ...
      content: (
        <div className="text-center">
          <p className="text-6xl mb-4">🎭</p>
          <h2 className="text-2xl font-bold mb-2">Tu género favorito</h2>
          <p className="text-4xl font-black text-cine-accent mt-4">{data.top_genre}</p>
    ...
    // Slide 2: Best & Worst
    {
      ...
      content: (
        <div className="text-center">
          <h2 className="text-2xl font-bold mb-6">Tus extremos</h2>
          <div className="space-y-4">
            {data.best_movie && (
              <Link to={`/movie/${data.best_movie.tmdb_id}`} className="flex items-center gap-4 bg-white/5 rounded-2xl p-4 hover:bg-white/10 transition">
                {data.best_movie.poster && (
                  <img src={imgUrl(data.best_movie.poster)} alt="" className="w-16 h-24 rounded-xl object-cover" />
                )}
                <div className="text-left flex-1">
                  <p className="text-xs text-cine-green mb-1">⬆️ Tu favorita</p>
                  <p className="font-bold">{data.best_movie.title}</p>
            ...
            {data.worst_movie && (
              <Link to={`/movie/${data.worst_movie.tmdb_id}`} className="flex items-center gap-4 bg-white/5 rounded-2xl p-4 hover:bg-white/10 transition">
                {data.worst_movie.poster && (
                  <img src={imgUrl(data.worst_movie.poster)} alt="" className="w-16 h-24 rounded-xl object-cover" />
                )}
                <div className="text-left flex-1">
                  <p className="text-xs text-red-400 mb-1">⬇️ Tu menos favorita</p>
                  <p className="font-bold">{data.worst_movie.title}</p>
                  <p className="text-cine-gold flex items-center gap-1 mt-1">
    ...
    // Slide 3: Monthly chart
    {
      ...
      content: (
        <div className="text-center">
          <p className="text-6xl mb-4">📅</p>
          <h2 className="text-2xl font-bold mb-2">Tu año mes a mes</h2>
    ...
    // Slide 4: Stats summary
    {
      ...
      content: (
        <div className="text-center">
          <p className="text-6xl mb-4">📊</p>
          <h2 className="text-2xl font-bold mb-6">Resumen {year}</h2>
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-white/5 rounded-2xl p-4">
              <Star className="mx-auto mb-2 text-cine-gold" size={24} />
              <p className="text-2xl font-black">{data.avg_rating || "—"}</p>
              <p className="text-xs text-cine-muted">Media de rating</p>
            </div>
            <div className="bg-white/5 rounded-2xl p-4">
              <Flame className="mx-auto mb-2 text-orange-400" size={24} />
              <p className="text-2xl font-black">{data.max_streak}</p>
              <p className="text-xs text-cine-muted">Mejor racha (días)</p>
            </div>
            <div className="bg-white/5 rounded-2xl p-4">
              <MessageSquare className="mx-auto mb-2 text-cine-accent" size={24} />
              <p className="text-2xl font-black">{data.total_reviews}</p>
              <p className="text-xs text-cine-muted">Reseñas escritas</p>
            </div>
            <div className="bg-white/5 rounded-2xl p-4">
              <TrendingUp className="mx-auto mb-2 text-cine-green" size={24} />
              <p className="text-2xl font-black">{data.total_ratings}</p>
              <p className="text-xs text-cine-muted">Valoraciones</p>
            </div>
          </div>
        </div>
      ),
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 pt-20 md:pt-20 pb-28">
      ...
      <div className="flex items-center gap-4 mb-6">
        <Link to="/profile" className="p-2 rounded-xl bg-cine-card border border-cine-border hover:bg-white/5">
          <ArrowLeft size={20} />
        </Link>
        <div className="flex-1">
          <h1 className="text-2xl font-bold">Tu Wrapped</h1>
          <p className="text-sm text-cine-muted">Resumen anual de tu cine</p>
        </div>
        {/* Year selector */}
        <div className="flex items-center gap-2 bg-cine-card border border-cine-border rounded-xl px-3 py-1.5">
          <button
      ...
      {loading ? (
        ...
        <>
          {/* Slide view */}
          <div className="relative overflow-hidden rounded-3xl bg-gradient-to-b border border-cine-border min-h-[420px]">
            <div className={`absolute inset-0 bg-gradient-to-b ${slides[slide]?.bg || "from-cine-bg to-cine-bg"} transition-all duration-500`} />
            <div className="relative z-10 p-6 flex flex-col justify-center min-h-[420px]">
